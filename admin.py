...
def my_form_post():
    ...
    uri = 'static/img/' + car
    nums = random.random()
    url = [{'_id':nums ,'url' : uri, 'name':'Rob'}]
    print(url)
    user.insert (url)
    for men in user.find({'_id':nums}): #ищет по Id
        print (men)

    jj = [{'title' : processed_text, 'name':'Rob'}]
    yse = [{'_id':5, 'url':uri}]
    user.insert(yse,jj)

    with open('static/datacar.json', 'w') as fat: # открывает json файл "W"- это команда на запись (write, read)
       jnn = json.dump(yse, fat, indent=2, ensure_ascii=False ) # mep это значение которому присвоено наше json значение из монги
